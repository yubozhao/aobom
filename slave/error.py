...
err_dic = {
    ...
    "99901" : "本地错误：SSLError",
    "99902" : "本地错误：URLError-TimeOut"
}

status_dic = {
    100 : 'todo',
    200 : '!exist',     #user doesn't exists
    201 : '!exist',     #status doesn't exists
    202 : 'fail-202',
    204 : 'fail-to-get',
    205 : 'fail-205',
    206 : 'fail-206',
...
class WeiboError:
    ...
    def handleError(self, exception):
        ...
        if isinstance(exception, SSLError):
            ...
            err_code = str(exception.error_code)
        else:
            if WeiboError.enableDebug:
                raise exception

        type = int( err_code[0:3] )
        ...
        r['type'] = type
        r['info'] = v
        r['status'] = str(exception)

        try:
            r['status'] = status_dic[type]
            h = err_handler[type]
            h()
    ...
    def defErrHandler(self,errTypeCode,handler):
        err_handler[errTypeCode] = handler